...
let rec f = function (* 不要定義削除ルーチン本体 (caml2html: elim_f) *)
  ...
  | Let((x, t), e1, e2, p) -> (* letの場合 (caml2html: elim_let) *)
      let e1' = f e1 in
      let e2' = f e2 in
      if effect e1' || S.mem x (fv e2') then Let((x, t), e1', e2', p) else
      (Format.eprintf "eliminating variable %s@." x;
       e2')
  | LetRec({ name = (x, t); args = yts; body = e1 }, e2, p) -> (* let recの場合 (caml2html: elim_letrec) *)
      let e2' = f e2 in
  ...
  | LetTuple(xts, y, e, p) ->
      ...
      let e' = f e in
      let live = fv e' in
      if List.exists (fun x -> S.mem x live) xs then LetTuple(xts, y, e', p) else
